...
At this stage, all data have been converted to numerical forms, been pruned, and is primed for testing.  As noted above, stratified sampliing is not nessicary for this corpus.  This leaves K-means cross validation for a number of models.  The models we will be testing are two of the more general models, elastic-net and MARS.  These were chosen for their generality and their tpyically acceptable performance as baseline models.  


```{r}

#Copasta from https://stackoverflow.com/questions/8805298/quickly-remove-zero-variance-variables-from-a-data-frame
...
fifa_predictors_no_missing <- fifa_predictors[,colSums(is.na(fifa_predictors)) == 0]
fifa_predictors_numerical <- fifa_predictors_no_missing[,sapply(fifa_predictors_no_missing, is.numeric)]
#fifa_predictors_numerical <- fifa_predictors_numerical[,-nearZeroVar(fifa_predictors_numerical)]
fifa_predictors_categorical <- fifa_predictors[,-sapply(fifa_predictors, is.numeric)]
fifa_targets_numerical <- fifa_targets[,sapply(fifa_targets, is.numeric)]
fifa_targets_categorical <- fifa_targets[,sapply(fifa_targets, is.numeric)]

################################
#TODO:
#Josh: add linear model, ridge regression, lasso regression
#

fifa_training_control <- trainControl(method = "cv", number = 5)



enet_grid <- expand.grid(.lambda = c(0, 0.01, 0.05, 0.1, 0.5, 1), .fraction = seq(0.05, 1, length = 20))

elastic_net_fit <- train(fifa_predictors_numerical, fifa_targets_numerical, preProcess = c("center", "scale", "BoxCox", "pca"), method = 'enet', tuneGrid = enet_grid, trControl = fifa_training_control)

PLS_fit <- train(
  x = fifa_predictors_numerical, 
  y = fifa_targets_numerical, 
  preProcess = c("center", "scale", "BoxCox", "pca"), 
  method = 'pls',
  trControl = fifa_training_control
)
